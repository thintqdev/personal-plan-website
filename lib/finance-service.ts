...
export async function getTransactionsWithPagination(
  ...
  try {
    ...
    const url = `${API_URL}/api/finance/transactions?${params.toString()}`;
...
/**
 * Get all transactions (legacy function for backward compatibility)
 */
export async function getTransactions(jarId?: string): Promise<Transaction[]> {
  try {
    const filters: TransactionFilters = {
      limit: 1000, // Large limit for backward compatibility
    };
